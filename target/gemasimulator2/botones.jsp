<a href="${url0}" class="btn btn-link btn-sm"> <span class="glyphicon glyphicon-home"></span> Menu</a>
<button type="submit" class="btn btn-primary btn-sm" onclick="execute('consultar')">
	<span class="glyphicon glyphicon-search"></span> Consultar
</button>
<button type="submit" class="btn btn-danger btn-sm" onclick="execute('procesar')">
	<span class="glyphicon glyphicon-cog"></span> Procesar
</button>
<button type="submit" class="btn btn-success btn-sm" onclick="execute('grabar')">
	<span class="glyphicon glyphicon-save"></span> Grabar
</button>
<button type="submit" class="btn btn-warning btn-sm" onclick="execute('update')">
	<span class="glyphicon glyphicon-check"></span> Modificar
</button>		
<button type="submit" class="btn btn-default btn-sm" onclick="execute('new')">
	<span class="glyphicon glyphicon-edit"></span> Nuevo
</button>

<%-- 
<div class="row">
    <div class="col-md-2">
		<select id="entidad" name="entidad" class="form-control">		  
		  <option value="0049">Santander</option>
		  <option value="0030">Banesto</option>
		  <option value="0075">Popular</option>
		  <option value="0624">OpenBank</option>
		  <option value="0616">SAN ECX</option>
		  <option value="0444">SISTEMA 4B</option>
		</select>
	</div>
</div>
--%>
