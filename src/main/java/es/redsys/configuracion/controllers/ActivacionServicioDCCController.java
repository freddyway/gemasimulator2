...
import es.redsys.configuracion.services.ActivacionServicioDCCService;
 ...
 */
@WebServlet(name = "ActivacionServicioDCC", urlPatterns = {"/serviciodcc"})

public class ActivacionServicioDCCController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger =  LoggerFactory.getLogger(ActivacionServicioDCCController.class);
		
	@EJB
	ActivacionServicioDCCService activacionServicioDCCService;
		
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ActivacionServicioDCCController() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.info("ActivacionServicioDCCController...");
		...
		try{
			...
			if("/serviciodcc".equals(servletPath)){					
				...
				if("consultar".equals(command)){
					mapParamRequest.put("listTR32084", activacionServicioDCCService.findAll());					
				}
				if("procesar".equals(command)){
					GemaOroService gemaOroService = new GemaOroService();
					respuesta = gemaOroService.processRequest(mapParamRequest,"32084",version,entidad);		
					mapParamRequest.put("codRespuesta", respuesta[8]);
				}
				
				if("grabar".equals(command)){
					activacionServicioDCCService.save(mapParamRequest,entidad,version);
					mapParamRequest.put("listTR32084", activacionServicioDCCService.findAll());
				}
				
				if("fila".equals(command)){
					String fila = (String) request.getParameter("rowid");
					mapParamRequest = activacionServicioDCCService.findById(fila);
					mapParamRequest.put("listTR32084", activacionServicioDCCService.findAll());
				}
				
				if("eliminar".equals(command)){					
					String fila = (String) request.getParameter("rowid");
					activacionServicioDCCService.delete(fila);					
				...
				if("update".equals(command)){
					...
					activacionServicioDCCService.update(mapParamRequest,id,entidad,version);
					mapParamRequest.put("codRespJPA","Modficacion Correcta!!!");
					mapParamRequest.put("listTR32084", activacionServicioDCCService.findAll());
				}
				
				pageResponse = "/tr32084.jsp";
			}
