...
import es.redsys.configuracion.services.EstadoTarjetaService;
 ...
 */
@WebServlet(name = "EstadoTarjetaServlet", urlPatterns = {"/estadotarj"})

public class EstadoTarjetaController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger =  LoggerFactory.getLogger(EstadoTarjetaController.class);
		
	@EJB
	EstadoTarjetaService service;
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EstadoTarjetaController() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.info("EstadoTarjetaController...");
		...
		try{
			...
			if("/estadotarj".equals(servletPath)){					
				...
				if("consultar".equals(command)){
					mapParamRequest.put("listTR31042", service.findAll());					
				}
				
				if("procesar".equals(command)){
					GemaOroService gemaOroService = new GemaOroService();
					respuesta = gemaOroService.processRequest(mapParamRequest,"31042",version,entidad);	
