...
import es.redsys.configuracion.services.AmpliarLimiteTarjetaService;
 ...
 */
@WebServlet(name = "AmpliarLimiteTarjetaServlet", urlPatterns = {"/amplimtarj"})

public class AmpliarLimiteTarjetaController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger =  LoggerFactory.getLogger(CargaPrepagoController.class);
		
	@EJB
	AmpliarLimiteTarjetaService service;
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AmpliarLimiteTarjetaController() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.info("AmpliarLimiteTarjetaController...");
		...
		try{
			...
			if("/amplimtarj".equals(servletPath)){					
				...
				if("consultar".equals(command)){
					mapParamRequest.put("listTR01026", service.findAll());					
				}
				
				if("procesar".equals(command)){
					GemaOroService gemaOroService = new GemaOroService();
					respuesta = gemaOroService.processRequest(mapParamRequest,"01026",version,entidad);	
