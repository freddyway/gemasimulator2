 ...
 */
@WebServlet(name = "ConsultaSaldoServlet", urlPatterns = {"/saldotarj"})

public class ConsultaSaldoController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger =  LoggerFactory.getLogger(ConsultaSaldoController.class);
     ...
     */
    public ConsultaSaldoController() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.info("ConsultaSaldoController...");
		...
		try{
			...
			if("/saldotarj".equals(servletPath)){					
				...
				if("consultar".equals(command)){
					mapParamRequest.put("listTR31245", consultaSaldoService.findAll());					
				}
				
				if("procesar".equals(command)){
					GemaOroService gemaOroService = new GemaOroService();
					respuesta = gemaOroService.processRequest(mapParamRequest,"31245",version,entidad);	
					mapParamRequest.put("codRespuesta", respuesta[8]);
				}
				
				if("grabar".equals(command)){
					consultaSaldoService.save(mapParamRequest,entidad,version);
					mapParamRequest.put("listTR31245", consultaSaldoService.findAll());
				}
				
				if("fila".equals(command)){
					String fila = (String) request.getParameter("rowid");										
					mapParamRequest = consultaSaldoService.findById(fila);					
				}
				
				if("eliminar".equals(command)){					
					String fila = (String) request.getParameter("rowid");
					consultaSaldoService.delete(fila);					
				...
				if("update".equals(command)){
					...
					consultaSaldoService.update(mapParamRequest,id,entidad,version);
					mapParamRequest.put("codRespJPA","Modficacion Correcta!!!");
					mapParamRequest.put("listTR31245", consultaSaldoService.findAll());
				}

				pageResponse = "/tr31245.jsp";
			}
